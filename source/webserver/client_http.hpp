...
namespace SimpleWeb {
    ...
    public:
        ...
        std::shared_ptr<Response> request(const std::string& request_type, const std::string& path, std::ostream& content, 
            ...
            size_t content_length= (size_t)content.tellp();
            ...
            try {
				...
				// DUCH -->
				// OBSOLETE CODE! Cancel of socket before inteerrupt is ok!
				/*
				...
				*/
                ...
                if(response->header.count("Content-Length")>0) {
                    boost::asio::read(*socket, response->content_buffer, 
                            boost::asio::transfer_exactly((size_t)stoull(response->header["Content-Length"])-num_additional_bytes));
                }
                    ...
                    do {
                        ...
                        line.pop_back();
                        length=(size_t)stoull(line, 0, 16);
                ...
                socket_error=true;
                throw std::invalid_argument(e.what()); // TODO: @@@
            }
    ...
    protected:       
        ...
        ClientBase(const std::string& host_port, unsigned short default_port) : 
			...
			if (boost::regex_match(host_port, sm, e)) {
                host=sm[1];
                port=default_port;
                if(sm[2]!="")
