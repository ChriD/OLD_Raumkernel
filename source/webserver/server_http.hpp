...
namespace SimpleWeb {
    ...
    public:
        ...
        typedef std::map<std::string, std::unordered_map<std::string, 
			// okay... i will be honest, ive no ide why one thing works on MSVC and not an GCC and the other on GCC and not on MSVC
			// i have to investigate and to fix the code if i have time to
			#ifdef _MSC_VER			
			std::function<void(std::ostream&, std::shared_ptr<ServerBase<boost::asio::ip::tcp::socket>::Request>)> > > resource_type; // < --works with msvc
			#endif
			#ifndef _MSC_VER		
			std::function<void(std::ostream&, std::shared_ptr<ServerBase<socket_type>::Request>)> > > resource_type; // <-- works with gcc
			#endif
    ...
    protected:
        ...
        ServerBase(unsigned short port, size_t num_threads, size_t timeout_request, size_t timeout_send_or_receive) : 
				...
				endpoint(boost::asio::ip::tcp::v4(), port), acceptor(m_io_service, endpoint, false), num_threads(num_threads),
                ...
                if(!ec) {
                    ...
                    if(request->header.count("Content-Length")>0) {
                        ...
                        boost::asio::async_read(*socket, request->content_buffer, 
                                boost::asio::transfer_exactly((size_t)stoull(request->header["Content-Length"])-num_additional_bytes), 
        ...
        void write_response(std::shared_ptr<socket_type> socket, std::shared_ptr<Request> request) {
            ...
            for(auto res_it: all_resources) {
				...
				boost::regex e(res_it->first);
                //std::smatch sm_res;
				boost::smatch sm_res;
                //if(std::regex_match(request->path, sm_res, e)) {
				if (boost::regex_match(request->path, sm_res, e)) {
                    if(res_it->second.count(request->method)>0) {
                        request->path_match=boost::move(sm_res);
